...
def get_blob_stats(my_image,smoothing_kernel_width,fluence_cut_off_low,fluence_cut_off_high):
    ...
    cluster_labels = set(lw[200:950,175:910].flatten())
...
def get_blob_stats2(my_image,smoothing_kernel_width,fluence_cut_off_low,fluence_cut_off_high):
    ...
    my_filtered_image = signal.fftconvolve(my_kernel,my_image,mode='same')-32.14

    #masking off regions with no photons

    binary_image = np.zeros(my_filtered_image.shape)
    binary_image[ my_filtered_image<8] = 0 
    binary_image[ my_filtered_image>8] = 1 

    #masking off regions with no photons.
    
    lw, num = measurements.label(binary_image)

    cluster_labels = set(lw[200:950,50:990].flatten())
...
def blobbifier(detectorObject,thisEvent):
	...
	if None == myImage:
		...
		my_dict = get_blob_stats2(myImage,smoothing_kernel_width, fluence_cut_off_low,fluence_cut_off_high)
		#my_dict['proj_2'] = sum(myImage[210:300],axis=0)
